...
//normal图片
@property (strong, nonatomic) NSArray *normalImageName;
@property (strong, nonatomic) PJXIconView *norImageView;
//selected图片
@property (strong, nonatomic) NSArray *selectedImageName;
@property (strong, nonatomic) PJXIconView *selImageView;
...
- (void)viewDidLoad
{
    ...
    self.normalImageName = @[@"v2_home",
                             @"v2_order",
                             @"freshReservation",
                             @"shopCart",
                             @"v2_my"];
    
    self.selectedImageName = @[@"v2_home_selected",
                               @"v2_order_selected",
                               @"freshReservation_selected",
                               @"shopCart_selected",
                               @"v2_my_selected"];
...
- (void)createCustomIcons:(NSDictionary *)containers
{
    ...
    if (items) {
        for (PJXAnimatedTabBarItem *item in self.tabBar.items) {
            ...
            [self.iconViews addObject:item.iconView];
            
            if (0 == index) {
                
                self.selImageView = self.iconViews[index];
                self.selImageView.icon.image = [UIImage imageNamed:self.selectedImageName[index]];
                
                [item selectedStateWithImageView:self.selImageView];
            }
...
- (void)tapHandler:(UIGestureRecognizer *)gesture
{
    ...
    if (self.selectedIndex != currentIndex) {
        ...
        self.norImageView = self.iconViews[self.selectedIndex];
        self.norImageView.icon.image = [UIImage imageNamed:self.normalImageName[self.selectedIndex]];
        //NSLog(@"currentIndex---%zd", currentIndex);
        //NSLog(@"selectedIndex---%zd", self.selectedIndex);
        
        self.selImageView = self.iconViews[currentIndex];
        self.selImageView.icon.image = [UIImage imageNamed:self.selectedImageName[currentIndex]];
        
        [animationItem playAnimationWithImageView:self.selImageView];
        
        PJXAnimatedTabBarItem *deselectItem = (PJXAnimatedTabBarItem *)items[self.selectedIndex];
        [deselectItem deselectAnimationWithImageView:self.norImageView];
        ...
        PJXAnimatedTabBarItem *selectItem = (PJXAnimatedTabBarItem *)items[self.selectedIndex];
        [selectItem selectedStateWithImageView:self.selImageView];
